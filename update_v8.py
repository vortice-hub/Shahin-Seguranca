...
COMMIT_MSG = "V8: Edicao de Itens, Btn Saida Vermelho e Menu Inicio"
# Mantendo a URL fixa para estabilidade total
...
<body class="bg-slate-50 text-slate-800">
    ...
    <footer class="mt-8 text-center text-xs text-slate-400 pb-8">
        &copy; 2026 Thay RH System. Versão 8.0 Pro
    </footer>
</body>
</html>
"""
...
<div class="grid grid-cols-2 gap-4 mb-8">
    ...
    <!-- Saída (Vermelho com Seta Vermelha) -->
    <a href="/saida" class="group relative bg-white border border-slate-200 hover:border-red-500 p-6 rounded-xl shadow-sm hover:shadow-md transition-all duration-300 flex flex-col items-center justify-center">
        <div class="w-12 h-12 bg-red-50 rounded-full flex items-center justify-center text-red-600 mb-3 group-hover:scale-110 transition">
            <i class="fas fa-arrow-up text-xl"></i> <!-- Seta Vermelha -->
        </div>
        <span class="font-bold text-slate-700 group-hover:text-red-600">Saída</span>
    </a>
</div>
            ...
            <div class="flex items-center gap-4">
                ...
                <div>
                    <div class="font-semibold text-slate-800 text-sm flex items-center gap-2">
                        {{ item.nome }}
                        <!-- Botão de Editar -->
                        <a href="/editar/{{ item.id }}" class="text-slate-300 hover:text-blue-500 transition px-1">
    ...
    <form action="/entrada" method="POST" class="p-6 space-y-5">
        <div>
            ...
            <datalist id="sugestoes">
                <option value="Camisa Polo"><option value="Calça Brim"><option value="Bota de Segurança"><option value="Jaleco">
            </datalist>
        </div>
        <div class="grid grid-cols-2 gap-4">
            <div>
        ...
        <button type="submit" class="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-4 rounded-lg shadow-md hover:shadow-lg transition-all transform active:scale-[0.98]">ADICIONAR AO ESTOQUE</button>
    </form>
</div>
{% endblock %}
"""
...
<div class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
    <form action="/saida" method="POST" class="p-6 space-y-5">
        ...
        <div>
            ...
            <input type="number" name="quantidade" min="1" value="1" class="w-full bg-slate-50 border border-slate-200 rounded-lg px-4 py-3 text-red-600 font-bold text-lg focus:outline-none focus:ring-2 focus:ring-red-500" required>
        </div>
        ...
        <button type="submit" class="w-full bg-red-600 hover:bg-red-700 text-white font-bold py-4 rounded-lg shadow-md hover:shadow-lg transition-all transform active:scale-[0.98]">CONFIRMAR ENTREGA</button>
    </form>
</div>
{% endblock %}
"""
...
<div class="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
    <table class="w-full text-left text-sm text-slate-600">
        <thead class="bg-slate-50 text-xs uppercase text-slate-400 font-bold">
            <tr><th class="px-6 py-3">Data</th><th class="px-6 py-3">Item</th><th class="px-6 py-3 text-right">Qtd</th></tr>
        </thead>
...
<div class="space-y-3">
    ...
    <div class="bg-white p-5 rounded-xl border border-slate-200 shadow-sm hover:shadow-md transition">
        <div class="flex justify-between items-start mb-2">
            ...
            <div class="bg-blue-50 text-blue-700 text-xs px-2 py-1 rounded font-bold">-{{ log.quantidade }} UN</div>
        </div>
        ...
        <div class="pt-3 border-t border-slate-100 text-sm text-slate-700 flex items-center gap-2">
            <i class="fas fa-tshirt text-slate-400"></i> {{ log.item_nome }} <span class="text-slate-400">|</span> {{ log.tamanho }} <span class="text-slate-400">|</span> {{ log.genero }}
        </div>
    </div>
    {% else %}
...
def git_update():
    try:
        ...
        print("\n>>> SUCESSO! Update V8 (Edição e Visual) Enviado. <<<")
...
def main():
    print(f"--- UPDATE V8: {PROJECT_NAME} ---")
    create_backup()
    ...
    write_file("templates/editar.html", FILE_EDITAR) # NOVO
