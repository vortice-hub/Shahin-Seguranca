...
COMMIT_MSG = "V64 Part 1: Backend QR Code - APIs de Token, Registro e Usuario Terminal"

# --- 1. ATUALIZAR ROUTES/PONTO.PY (Adicionar Lógica de Token e API) ---
FILE_BP_PONTO = """
...
def registrar_leitura_terminal():
    # Apenas o usuário TERMINAL pode registrar pontos para outros
    if current_user.role != 'Terminal' and current_user.role != 'Master':
        return jsonify({'error': 'Acesso negado. Apenas Terminal.'}), 403
    ...
    try:
        # Tenta decifrar o token. max_age=35 aceita tokens de até 35 segundos
        dados = s.loads(token, max_age=35)
        user_id_alvo = dados['user_id']
        
        user_alvo = User.query.get(user_id_alvo)
        if not user_alvo:
            return jsonify({'error': 'Usuário inválido'}), 404
            
        # LÓGICA DE REGISTRO (A mesma do registrar_ponto manual)
        ...
        novo = PontoRegistro(
            ...
            tipo=proxima,
            latitude='QR-Code', # Marca como QR Code
        ...
        return jsonify({'error': 'QR Code expirado! Gere um novo.'}), 400
    except BadSignature:
        return jsonify({'error': 'QR Code inválido ou falsificado.'}), 400
    except Exception as e:
        return jsonify({'error': f'Erro interno: {str(e)}'}), 500

# --- ROTAS DE INTERFACE (MANTIDAS/ADAPTADAS) ---
...
def registrar_ponto():
    ...
    # Helper rapido para data por extenso (ja que nao importamos o utils completo aqui pra evitar circular, mas idealmente estaria la)
    ...
    return render_template('ponto/registro.html', proxima_acao=prox, hoje_extenso=hoje_extenso, pontos=pontos, bloqueado=bloqueado, motivo=motivo)

# --- OUTRAS ROTAS DO PONTO (SEM ALTERAÇÃO) ---
...
# --- 2. SCRIPT DE INICIALIZAÇÃO DO USUÁRIO TERMINAL ---
FILE_BOOT_TERMINAL = """
...
def run_terminal_setup():
    print("Executando criação do usuário Terminal...")
    try:
        subprocess.run([sys.executable, "setup_terminal.py"], check=True)
    except Exception as e:
        print(f"Erro ao rodar setup_terminal: {e}")
...
def git_update():
    try:
        ...
        print("\n>>> SUCESSO V64 PARTE 1 CORRIGIDA! <<<")
    except Exception as e: print(f"Git: {e}")

def self_destruct():
    try: os.remove(os.path.abspath(__file__))
    except: pass
    try: os.remove("setup_terminal.py")
    except: pass

def main():
    print(f"--- UPDATE V64 PARTE 1 (BACKEND FIX): {PROJECT_NAME} ---")
    
    write_file("app/routes/ponto.py", FILE_BP_PONTO)
    write_file("setup_terminal.py", FILE_BOOT_TERMINAL)
    
    # Roda o script que cria o usuario no banco
    run_terminal_setup()
