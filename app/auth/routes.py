...
def auto_cadastro():
    ...
    if request.method == 'POST':
        ...
        cpf = cpf_input.replace('.', '').replace('-', '').strip()
        ...
        if password:
            ...
            while User.query.filter_by(username=username).first(): 
                username = gerar_login_automatico(pre.nome_previsto) + str(db.session.query(User).count())
            ...
            novo_user = User(
                ...
                # --- NOVOS DADOS COPIADOS (JORNADA E EMPRESA) ---
                ...
                horario_entrada=pre.horario_entrada, # Mantido por legado
