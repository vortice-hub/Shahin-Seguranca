...
def gerar_pdf_espelho_mensal(user, mes_ano_str):
    ...
    for dia in range(1, last_day + 1):
        ...
        # Formatação
        saldo_fmt = format_minutes_to_hm(saldo)
        trab_fmt = format_minutes_to_hm(trabalhado).replace('-', '')
        
        # Adiciona Linha
        dados.append([
            dt_atual.strftime('%d/%m'),
            dias_semana[dt_atual.weekday()],
            horarios_str,
            trab_fmt,
            saldo_fmt,
            status
    ...
    style = TableStyle([
        ...
        ('BACKGROUND', (0,-1), (-1,-1), colors.lightgrey), # Linha total
        ('GRID', (0,0), (-1,-1), 0.5, colors.grey),
        ('FONTSIZE', (0,1), (-1,-1), 8),
    ])
    t.setStyle(style)
    
    # Desenha tabela no Canvas
