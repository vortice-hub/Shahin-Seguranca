...
from app.documentos.utils import gerar_certificado_entrega
...
def dashboard_documentos():
    """Painel administrativo de documentos."""
...
def revisao_auditoria():
    """Rota de Auditoria de Recebimento (Digital Signatures)."""
    usuarios = User.query.filter(User.role != 'Terminal').order_by(User.real_name).all()
    auditores = []
    for user in usuarios:
        assinaturas_user = AssinaturaDigital.query.filter_by(user_id=user.id).order_by(AssinaturaDigital.data_assinatura.desc()).all()
        assinaturas_formatadas = []
        for a in assinaturas_user:
            # Tenta encontrar a referência do documento (ex: Mês do Holerite)
            ref = "ID: " + str(a.documento_id)
            if a.tipo_documento == 'Holerite':
                h = Holerite.query.get(a.documento_id)
                if h: ref = h.mes_referencia
            
            assinaturas_formatadas.append({
                'id': a.id, 'tipo': a.tipo_documento, 'referencia': ref,
                'data': a.data_assinatura, 'ip': a.ip_address
            })
        
        auditores.append({
            'user': user,
            'total': len(assinaturas_user),
...
def baixar_certificado(id):
    """Gera o PDF do certificado de entrega digital."""
...
def revisao_holerites():
    """Lista holerites que a IA não conseguiu identificar."""
...
def vincular_holerite():
    """Vincula manualmente um holerite a um funcionário."""
    h_id = request.form.get('holerite_id'); u_id = request.form.get('user_id')
