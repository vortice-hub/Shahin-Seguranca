...
COMMIT_MSG = "V39: Refatoracao Completa - Arquitetura Modular (Blueprints)"

# --- ESTRUTURA DE DIRETÓRIOS ---
# Criamos as pastas necessarias
dirs = [
    "app",
    "app/routes",
    "app/templates" # Vamos mover os templates pra ca se nao estiverem
]
...
# 1. run.py (Novo Ponto de Entrada)
FILE_RUN = """
...
# 2. Procfile (Atualizado para usar run:app)
FILE_PROCFILE = """web: gunicorn run:app"""

# 3. app/__init__.py (A Fábrica do App)
FILE_INIT = """
import os
...
# 4. app/models.py (Tabelas do Banco)
FILE_MODELS = """
...
# 5. app/utils.py (Motor de Calculo e Helpers)
FILE_UTILS = """
...
@login_required
def controle_uniforme(): return render_template('controle_uniforme.html', itens=ItemEstoque.query.all()) if current_user.role == 'Master' else redirect(url_for('main.dashboard'))
...
def move_templates():
    # Se os templates estiverem na raiz/templates, movemos para app/templates
    if os.path.exists("templates") and not os.path.exists("app/templates"):
        shutil.move("templates", "app/templates")
        print("Templates movidos para app/templates")
    elif os.path.exists("templates"):
        # Se ambos existem, mescla
        for item in os.listdir("templates"):
            shutil.move(os.path.join("templates", item), os.path.join("app/templates", item))
        shutil.rmtree("templates")
        print("Templates mesclados.")
...
def git_update():
    try:
        ...
        print("\n>>> SUCESSO! ESTRUTURA MODULARIZADA ENVIADA <<<")
...
def main():
    print(f"--- INICIANDO MODULARIZAÇÃO: {PROJECT_NAME} ---")
    ...
    write_file("app/routes/__init__.py", "") # Init vazio para pacote
    write_file("app/routes/auth.py", FILE_BP_AUTH)
    write_file("app/routes/main.py", FILE_BP_MAIN)
    write_file("app/routes/ponto.py", FILE_BP_PONTO)
    write_file("app/routes/admin.py", FILE_BP_ADMIN)
    write_file("app/routes/estoque.py", FILE_BP_ESTOQUE)
    
    # 2. Move Templates
    move_templates()
    
    # 3. Remove app.py antigo se existir (para evitar confusao)
    if os.path.exists("app.py"):
        os.remove("app.py")
        print("Arquivo app.py antigo removido (substituído por run.py + app/ package)")
